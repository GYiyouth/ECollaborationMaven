...
//        StudentScoreVO studentScoreVO2 = BeanFactory.getBean("studentScoreVO",StudentScoreVO.class);
    ...
    public StudentScoreVO getStudentScoreVOByStudentAndProjectAccessType(ProjectAccessTypeVO projectAccessTypeVO, StudentVO studentVO) {
        if (projectAccessTypeVO == null || studentVO == null) {
            throw new NullPointerException("projectAccessTypeVO/studentVO is null");
        } else {
            return (StudentScoreVO) hibernateTemplate.find("from StudentScoreVO ss where ss.projectAccessTypeVO = ? and ss.studentVO = ?", projectAccessTypeVO, studentVO).get(0);
        }
    }
}
